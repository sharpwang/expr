...
public class InternalBaiWei extends InternalSelection implements Selectable {

	InternalBaiWei(Library3D lib) {
		super(lib);
		...
		for(int i=0; i<=9; i++){
			for(int j=0; j<=9; j++){
				for(int k=0; k<=9; k++){
					...
					if(op.equals(">") && i > value.getFloat()){
						__list.add(ball);
					}
					else if(op.equals(">=") && i >= value.getFloat()){
						__list.add(ball);
